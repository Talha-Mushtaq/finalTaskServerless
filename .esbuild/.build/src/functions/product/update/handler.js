...
// src/functions/product/update/handler.ts
__export(exports, {
  main: () => main
});
...
var getDataFunc = async (params) => {
  const response = await dynamoDB.query(params).promise();
  return response;
};
var updateDataFunc = async (params) => {
  const response = await dynamoDB.update(params).promise();
  return response;
};

// src/functions/product/update/handler.ts
var updateProduct = async (event) => {
  try {
    ...
    const query = {
      ...
      UpdateExpression: "SET title = :title , description = :description , price = :price , categories = :categories",
...
var main = middyfy(updateProduct);
